 ...
 * Vercel Cron trigger for syncing all active Meta (Instagram/Facebook) connections.
  ...
  try {
    ...
    if (platformError || !platform) {
      return NextResponse.json(
        { error: 'Meta platform not found in platforms table' },
        { status: 404 },
      )
    }
    ...
    if (connError) {
      console.error('[cron/sync-meta] Error fetching connections:', connError.message)
    ...
    const settled = await Promise.allSettled(
      connections.map(async (conn) => {
        ...
        const syncResponse = await fetch(`${origin}/api/sync/meta`, {
          method: 'POST',
    ...
    console.error('[cron/sync-meta] Error:', message)
