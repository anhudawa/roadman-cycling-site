...
/**
 * GET /api/cron/sync-beehiiv
 * Vercel Cron trigger for syncing all active Beehiiv connections.
  ...
  try {
    ...
    if (platformError || !platform) {
      return NextResponse.json(
        { error: 'Beehiiv platform not found in platforms table' },
        { status: 404 },
      )
    }

    // ---- Find all active Beehiiv connections -------------------------------
    ...
    if (connError) {
      console.error('[cron/sync-beehiiv] Error fetching connections:', connError.message)
    ...
    if (!connections || connections.length === 0) {
      return NextResponse.json({
        ok: true,
        message: 'No active Beehiiv connections to sync',
        synced: 0,
    ...
    const settled = await Promise.allSettled(
      connections.map(async (conn) => {
        ...
        const syncResponse = await fetch(`${origin}/api/sync/beehiiv`, {
          method: 'POST',
    ...
    console.error('[cron/sync-beehiiv] Error:', message)
