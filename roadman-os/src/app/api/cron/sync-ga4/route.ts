 ...
 * Vercel Cron trigger for syncing all active GA4 connections.
  ...
  try {
    ...
    if (platformError || !platform) {
      return NextResponse.json(
        { error: 'GA4 platform not found in platforms table' },
        { status: 404 },
      )
    }

    // ---- Find all active GA4 connections ----------------------------------
    ...
    if (connError) {
      console.error('[cron/sync-ga4] Error fetching connections:', connError.message)
    ...
    if (!connections || connections.length === 0) {
      return NextResponse.json({
        ok: true,
        message: 'No active GA4 connections to sync',
        synced: 0,
    ...
    const settled = await Promise.allSettled(
      connections.map(async (conn) => {
        ...
        const syncResponse = await fetch(`${origin}/api/sync/ga4`, {
          method: 'POST',
    ...
    console.error('[cron/sync-ga4] Error:', message)
