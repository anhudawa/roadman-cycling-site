 ...
 * Vercel Cron trigger for syncing all active LinkedIn connections.
  ...
  try {
    ...
    if (platformError || !platform) {
      return NextResponse.json(
        { error: 'LinkedIn platform not found in platforms table' },
        { status: 404 },
      )
    }

    // ---- Find all active LinkedIn connections ------------------------------
    ...
    if (connError) {
      console.error('[cron/sync-linkedin] Error fetching connections:', connError.message)
    ...
    if (!connections || connections.length === 0) {
      return NextResponse.json({
        ok: true,
        message: 'No active LinkedIn connections to sync',
        synced: 0,
    ...
    const settled = await Promise.allSettled(
      connections.map(async (conn) => {
        ...
        const syncResponse = await fetch(`${origin}/api/sync/linkedin`, {
          method: 'POST',
    ...
    console.error('[cron/sync-linkedin] Error:', message)
