 ...
 * Vercel Cron trigger for syncing all active Spotify connections.
  ...
  try {
    ...
    if (platformError || !platform) {
      return NextResponse.json(
        { error: 'Spotify platform not found in platforms table' },
        { status: 404 },
      )
    }

    // ---- Find all active Spotify connections -------------------------------
    ...
    if (connError) {
      console.error('[cron/sync-spotify] Error fetching connections:', connError.message)
    ...
    if (!connections || connections.length === 0) {
      return NextResponse.json({
        ok: true,
        message: 'No active Spotify connections to sync',
        synced: 0,
    ...
    const settled = await Promise.allSettled(
      connections.map(async (conn) => {
        ...
        const syncResponse = await fetch(`${origin}/api/sync/spotify`, {
          method: 'POST',
    ...
    console.error('[cron/sync-spotify] Error:', message)
