...
// ---------------------------------------------------------------------------
async function main() {
  console.log("ðŸ§Š Answer Capsule Generator");
  ...
  console.log(`ðŸ“ ${posts.length} posts to process\n`);
...
Return ONLY the capsule text, no quotes, no labels, no formatting.`;
    ...
    try {
      ...
      if (!dryRun) {
        ...
        if (wordCount < 20 || wordCount > 100) {
          console.log(`  $š  Capsule word count (${wordCount}) outside range, keeping anyway`);
        }
        ...
        console.log(`  $œ… Capsule added (${wordCount} words)`);
  ...
  console.log(`\n$œ… Done: ${processed} processed, ${skipped} skipped`);
  printCostSummary();
}
