...
function loadEpisode(slug: string): LoadedEpisode | null {
  ...
  if (!fs.existsSync(filePath)) {
    console.error(`$ùå Episode file not found: ${filePath}`);
    return null;
  }
  ...
  try {
    ...
    console.error(`$ùå Failed to load episode "${slug}": ${msg}`);
    return null;
  }
}
...
function findLatestEpisode(): LoadedEpisode | null {
  ...
  if (!fs.existsSync(podcastDir)) {
    console.error("$ùå Podcast content directory not found");
    return null;
  }
...
function toEpisodeInput(ep: LoadedEpisode): EpisodeInput | null {
  ...
  if (!transcript) {
    console.log(`   $ö† No transcript in frontmatter for "${slug}" $Äî skipping`);
    return null;
  }
...
async function processEpisode(ep: LoadedEpisode, state: RepurposeState): Promise<boolean> {
  ...
  if (!force && outputExists(input.episodeNumber, input.slug)) {
    console.log(`   $è≠  Output already exists $Äî skipping (use --force to overwrite)`);
    return false;
  }

  // Log episode info
  console.log(`   üìÑ Title:  ${input.title}`);
  console.log(`   üè∑  Pillar: ${input.pillar}`);
  console.log(`   üéô  Type:   ${input.type}`);
  if (input.guest) {
    console.log(`   üë§ Guest:  ${input.guest}${input.guestCredential ? ` $Äî ${input.guestCredential}` : ""}`);
  }
  ...
  if (!skipBlog) {
    ...
    if (blog) {
      result.blog = blog;
      console.log(`$úì ${blog.slug}`);
    } else {
      console.log("$úó failed");
    }
  } else {
    console.log("   üìù Blog post... skipped (--skip-blog)");
  }

  // --- Social ---
  if (!skipSocial) {
    process.stdout.write("   üì± Social posts... ");
    const social = await generateSocialPosts(input);
    if (social) {
      result.social = social;
      const tweetCount = social.twitter?.tweets?.length ?? 0;
      console.log(`$úì ${tweetCount} tweets + IG + LinkedIn + FB`);
    } else {
      console.log("$úó failed");
    }
  } else {
    console.log("   üì± Social posts... skipped (--skip-social)");
  }

  // --- Quotes ---
  if (!skipQuotes) {
    process.stdout.write("   üí$ Quotes... ");
    const quotes = await extractQuotes(input.transcript, input.title, input.guest);
    console.log(`$úì ${quotes.length} extracted`);

    if (quotes.length > 0) {
      // Render quote cards (skip in dry-run $Äî no image fetch or rendering)
      if (!dryRun) {
        // Guest image
        ...
        if (imageBuffer) {
          ...
          console.log("$ö† none found $Äî cards will use solid background");
        }

        process.stdout.write("   üé® Quote cards... ");
        ...
        console.log(`$úì ${cardPaths.length} cards rendered`);
      } else {
        console.log("   üé® Quote cards... skipped (dry-run)");
        result.quotes = { extracted: quotes, cardPaths: [] };
      }
    }
  } else {
    console.log("   üí$ Quotes... skipped (--skip-quotes)");
  }
  ...
  if (!dryRun) {
    console.log(`   $úÖ Wrote ${writtenPaths.length} files`);
    ...
    console.log(`   üè∑  [dry-run] Would write ${writtenPaths.length} files`);
  }
...
async function main() {
  ...
  console.log("üéô  Roadman Podcast Repurpose");
  ...
  if (episodeFlag) {
    ...
    if (!ep) {
      console.error("$ùå Could not find any episode with a publishDate");
      process.exit(1);
    }
    console.log(`üìã Latest episode: ${ep.slug}`);
    episodes = [ep];
  } else if (auto) {
    episodes = findUnprocessedEpisodes(state.processedEpisodeSlugs);
    console.log(`üìã Unprocessed episodes (last 7 days): ${episodes.length}`);
    ...
    console.error("$ùå No mode specified. Usage:");
  ...
  // Summary
  console.log("\n" + "=".repeat(50));
  console.log("üìä REPURPOSE COMPLETE");
