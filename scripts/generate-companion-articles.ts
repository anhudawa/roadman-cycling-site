...
// ---------------------------------------------------------------------------
async function main() {
  console.log("📝 Podcast Companion Article Generator");
  ...
  console.log(`🏆 Top ${ranked.length} episodes selected:\n`);
...
The seoDescription must be 150-160 characters.`;
      ...
      if (!dryRun) {
        ...
        if (jsonMatch) {
          try {
            ...
            console.log("  ⚠ Could not parse metadata JSON, using defaults");
          }
        }
  ...
  console.log(`\n✅ Done: ${processed} companion articles generated`);
  printCostSummary();
}
