...
} from "../src/lib/seo/app-search-measurement";
...
if (!baselinePath || !currentPath) {
  console.error(
    "Usage: npm run seo:app:compare -- <baseline.json> <current.json> [--output report.md]",
  );
  process.exitCode = 1;
} else {
  const read = (path: string): AppSearchSnapshot =>
    JSON.parse(readFileSync(resolve(path), "utf8")) as AppSearchSnapshot;
  const markdown = renderAppSearchComparisonMarkdown(
    compareAppSearchSnapshots(read(baselinePath), read(currentPath)),
  );
  if (outputPath) {
