...
export async function generateBlogPost(
  ...
  if (!apiKey) {
    console.error("  ✗ ANTHROPIC_API_KEY not set — skipping blog generation");
    return null;
  }
  ...
  try {
    ...
    if (!jsonBlockMatch) {
      console.error(`  ✗ Could not find JSON block in response for: ${episode.title}`);
      return null;
    }
    ...
    try {
      ...
      console.error(`  ✗ Failed to parse metadata JSON for: ${episode.title}`);
      return null;
    }
    ...
    if (!blogBody) {
      console.error(`  ✗ Empty blog body for: ${episode.title}`);
      return null;
    }
    ...
    console.error(`  ✗ Blog generation failed for "${episode.title}": ${msg}`);
    return null;
  }
}
