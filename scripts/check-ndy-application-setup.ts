...
async function main() {
  ...
  if (!table.rows[0]?.name) throw new Error("Run migration 0063 before enabling the application workflow");
  const base = `https://api.beehiiv.com/v2/publications/${process.env.BEEHIIV_PUBLICATION_ID}`;
  const headers = { Authorization: `Bearer ${process.env.BEEHIIV_API_KEY}` };
  const response = await fetch(`${base}/automations/${process.env.BEEHIIV_AUTOMATION_NDY_APPLICATION}`, { headers, signal: AbortSignal.timeout(10_000) });
