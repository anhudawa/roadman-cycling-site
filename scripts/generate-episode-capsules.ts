...
async function generateCapsuleForEpisode(
  ...
  if (wordCount < 40 || wordCount > 140) {
    console.warn(
      `   ⚠️  Capsule length out of range (${wordCount} words). Skipping.`,
    );
    return null;
  }
...
async function main() {
  console.log(`💬 Generate episode answer capsules`);
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    try {
      ...
      if (!capsule) {
        if (!dryRun) {
          skippedInvalid++;
          console.log(`   ⚠️  Skipped (invalid capsule)`);
        }
        continue;
      }
      ...
      succeeded++;
      console.log(`   ✓ ${capsule.split(/\s+/).length} words: "${capsule.slice(0, 80)}${capsule.length > 80 ? "…" : ""}"`);
    } catch (err) {
      failed++;
