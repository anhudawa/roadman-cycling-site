...
/** RFC-5322 lite $Äî rejects `foo@`, `@bar`, and other common fat-finger failures. */
...
function saveDraft(state: DraftState) {
  ...
  try {
    ...
    /* quota exceeded or storage unavailable $Äî ignore */
  }
}
...
const GOALS = [
  { value: "Race or event with a date", emoji: "üèÅ" },
  { value: "Hit a specific power number", emoji: "$ö°" },
  { value: "Stop getting dropped on group rides", emoji: "üí®" },
  { value: "Lose weight without losing power", emoji: "$öñÔ∏è" },
  { value: "Get structured after years of winging it", emoji: "üìã" },
];
...
const FRUSTRATIONS = [
  { value: "Plateaued $Äî stuck at a number I can't shift", emoji: "üìâ" },
  { value: "No structure $Äî making it up as I go", emoji: "üé≤" },
  { value: "Lost motivation $Äî can't stay consistent", emoji: "üò©" },
  { value: "Injury or comeback $Äî trying to get back", emoji: "ü©π" },
  { value: "Training hard but not seeing results", emoji: "üò§" },
];
...
export function CohortApplicationForm() {
  ...
  useEffect(() => {
    ...
    if (!draft) return;
    // Don't restore to the submitted step $Äî force user back into flow
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Lead event (FB Pixel + GA) $Äî attribution for ad spend
      trackLead(
        trimmedEmail,
        data.persona,
        cohortState.phase,
      ...
      // Success $Äî wipe the draft so next visit starts fresh
      clearDraft();
      setStep("submitted");
    } catch (err) {
      const msg =
    ...
    <div className="relative">
      ...
      <AnimatePresence mode="wait">
        ...
        {step === "submitted" && (
          ...
          >
            <div className="text-5xl mb-4">{isWaitlist ? "$úÖ" : "üéØ"}</div>
