...
export function ResendLinkForm({ initialEmail }: { initialEmail: string }) {
  ...
  if (sent) {
    return (
      <div className="rounded-lg border border-coral/30 bg-coral-muted p-6 text-off-white text-left">
        <p className="font-heading uppercase tracking-wider text-coral mb-2">
          Fresh link sent
        </p>
        <p>
          If <span className="text-coral">{email}</span> has Blood Engine access,
          a new sign-in link is on its way. The link expires in 30 minutes.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4 text-left">
      ...
      <Button type="submit" disabled={loading} size="lg" className="w-full">
        {loading ? "Sending…" : "Send a fresh link"}
      </Button>
