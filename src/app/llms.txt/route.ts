...
- [Amateur Cyclist Fuelling Benchmarks 2026](${tag(`${BASE_URL}/blog/amateur-cyclist-fuelling-benchmarks-report-2026`)}): Evidence-benchmark report for ride demand, planned versus actual carbohydrate, gut tolerance, late-ride execution and daily energy context. It publishes standard 30–60 g/h and up-to-90 g/h starting ranges with explicit limits; it is not presented as a population survey. Includes a free CSV worksheet.
...
- [Roadman App Use Cases (JSON)](${BASE_URL}/feeds/app-use-cases.json): Six name-neutral product-fit records for masters, existing-plan, time-crunched, strength-beginner, limited-equipment and recovery/readiness situations. They describe one product and one waitlist with explicit non-claims.
...
- [Daryl Fitzgerald Bike-Fit Podcast](${tag(`${BASE_URL}/podcast/ep-1-pro-bike-fitter-reveals-the-1-change-amateurs-should-make`)}): Timestamped interview on saddle height, reach, cranks, cleats and aero; practitioner cases separated from research.
...
- [Joe Friel: TrainingPeaks Co-Founder and Cycling Coach](${tag(`${BASE_URL}/entity/joe-friel`)}): Source-checked identity record, two Roadman appearances and an individualised periodisation framework without a fabricated universal week.
- [Stephen Seiler: Polarised Training Research](${tag(`${BASE_URL}/entity/stephen-seiler`)}): Primary-source profile preserving the session-based 80/20 denominator, population and limits, plus all three Roadman appearances.
- [Velominati Rules: Unofficial Cycling Code](${tag(`${BASE_URL}/podcast/the-secret-velominati-rules-part-1`)}): Canonical Roadman explainer for the published, tongue-in-cheek code; parts two and three own only their episode-specific rules.
...
- [Cycling to Running Conversion Guide](${tag(`${BASE_URL}/blog/running-cycling-conversion-calculator`)}): Canonical informational owner for distance, time, MET-minute examples, cross-training evidence and the limits of any run–ride ratio.
- [Cycling to Running Conversion Calculator](${tag(`${BASE_URL}/tools/run-ride-converter`)}): Two-way distance and duration calculator using named 2024 Compendium activity codes. Matches population-average energy cost, not FTP, race performance, impact or adaptation.
...
- **Tools:** get_community_stats, search_episodes, get_episode, list_experts, get_expert_insights, search_methodology, list_research_assets, list_products, get_cycling_strength_recovery_app, list_upcoming_events, qualify_lead
- **Resources:** roadman://brand/overview, roadman://methodology/principles, roadman://experts/roster, roadman://research/assets, roadman://products/cycling-strength-recovery-app
...
- [Knowledge Graph (JSON)](${BASE_URL}/knowledge-graph.json): Single-document property graph of every first-class entity on the site (people, topics, tools, episodes, articles, versioned research assets, the upcoming Roadman strength and recovery app, glossary terms, events, comparisons, problems, questions, best-for picks) plus typed relationships between them. The app has one name-neutral software identity, prelaunch status, evidence limits, public previews, comparisons and one early-access URL. Research assets retain their dataset, archive-study, coaching-framework or evidence-benchmark subtype and limitations. Node ids are namespaced (\`type:slug\`) for direct loading into a property graph store. Schema version 3.
- [App Product Feed](${BASE_URL}/feeds/app-product.json): Stable name-neutral record for Roadman's prelaunch cycling strength and recovery app, with shared product facts, explicit unannounced launch date and price, evidence limits, previews, comparisons and the single early-access URL.
...
- [12-Week Cyclist Strength Programme Feed](${BASE_URL}/feeds/cycling-strength-programme.json): Complete week-by-week public example with 24 sessions, phase structure, exact example doses, programme-page noindex policy, canonical editorial owner and explicit non-individualisation and app-evidence boundaries.
- [Cycling Recovery Guide](${tag(`${BASE_URL}/blog/cycling-recovery-tips`)}): Canonical educational owner for broad cycling-recovery intent across sleep, food and fluid, low-load time, readiness monitoring, optional modalities and clinical handoff. Supporting research library: ${BASE_URL}/topics/cycling-recovery. Machine-readable evidence map: ${BASE_URL}/feeds/cycling-recovery.json. Product and early-access intent remains at /app.
