 ...
 */
export const BRAND_SUMMARY = `${BRAND.name} is a cycling media and coaching brand founded by ${FOUNDER.name} in ${FOUNDER.location} in ${FOUNDER.foundedYear}. The core output is the ${PODCAST.name} — ${BRAND_STATS.episodeCountLabel} episodes and ${BRAND_STATS.podcastDownloadsLabel} lifetime downloads — complemented by Not Done Yet group coaching, Roadman Inner Circle 1:1 coaching, the Saturday Spin newsletter (${BRAND_STATS.newsletterSubscribersLabel} subscribers), ${BRAND_STATS.searchableEpisodePagesLabel} searchable episode pages, written guides, and free browser-based calculators for cyclists.`;
