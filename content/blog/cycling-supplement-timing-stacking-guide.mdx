...
answerCapsule: >-
  Take iron on an empty stomach with vitamin C, separated from coffee by at
  least two hours — coffee reduces iron absorption by up to 60%. Time caffeine
  to peak 45-60 minutes before the hardest effort, not at the ride start. Take
  creatine at any consistent time daily. Vitamin D goes with a fat-containing
  meal. Magnesium glycinate before bed supports both sleep and mineral
  replenishment.
keyTakeaways:
...
**The recovery mega-stack.** BCAAs, glutamine, HMB, tart cherry extract, and whatever else fits on the label. If daily protein intake is already sufficient, extra amino-acid products may add no useful outcome. HMB findings depend on the population and context. Tart cherry may support selected muscle-function and inflammatory outcomes, but the newest pooled analysis found no significant soreness effect and products are not automatically equivalent. The [tart-cherry evidence guide](/blog/cycling-tart-cherry-juice-recovery-guide) separates that signal from the stack marketing.
