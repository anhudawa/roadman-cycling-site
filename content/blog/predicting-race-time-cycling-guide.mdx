...
roadmanView:
  - "Strava's estimated time is a popularity contest, not a prediction. It has no knowledge of your power, weight, or position. A physics-based model uses the forces actually acting on you — gravity, drag, rolling resistance — which is why it lands within 2-3 per cent."
  - "Use the Roadman Race Time Predictor tool with honest inputs: a tested FTP, accurate total system weight, and a real course profile. The gap between predicted and actual time from a past race exposes your real-world CdA."
  - "A flat 40km TT prediction is dominated by CdA and power. A 0.03 m-squared CdA difference changes the finish time by 90 seconds or more. That is why position matters more than fitness for most amateur time trialists."
---
