...
answerCapsule: >-
  Overtraining exists on a spectrum: functional overreaching recovers in days,
  non-functional overreaching takes weeks, and true overtraining syndrome can
  sideline a rider for months. HRV trending below personal baseline for five or
  more consecutive days is the most reliable early warning. Declining power at
  the same RPE across a week — not a single bad session — signals trouble. If
  performance has not returned after two full weeks of reduced training and
  sleep focus, get blood work checking cortisol, testosterone, ferritin, and
  thyroid function.
keyTakeaways:
