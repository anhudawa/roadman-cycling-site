...
answerCapsule: >-
  Set up a race-day head unit screen with three fields only: three-second power,
  lap normalised power, and heart rate. The first 20 minutes is where adrenaline
  masks effort — 280 watts feels like 240 — so set a power ceiling to prevent
  overcooking the opening. Use normalised power rather than average to assess
  race cost. In sprints and decisive moves, ignore the screen and race by
  instinct.
keyTakeaways:
