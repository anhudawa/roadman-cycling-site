...
answerCapsule: >-
  Watts per kilo is FTP divided by body mass and is the primary predictor of
  climbing performance on gradients above 6%. On flat terrain, raw watts
  dominate. A recreational club rider sits between 2.0 and 2.8 W/kg; a
  strong amateur racer between 3.5 and 4.2 W/kg. Most riders try to improve
  the ratio by losing weight first — that is backwards. Raising FTP through
  structured training is more sustainable and effective. Masters cyclists
  lose roughly 1% of VO2max per year from the mid-thirties, so age-adjusted
  benchmarks matter.
keyTakeaways:
