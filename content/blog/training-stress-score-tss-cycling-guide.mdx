...
roadmanView:
  - "TSS distils an entire ride into a single number — but that number is only useful if you know what it actually measures. One hour at FTP equals exactly 100 TSS. That gives every session a comparable number, and it is properly useful for planning your week. The problems start when riders chase the number instead of using it."
  - "TSS per hour is a better intensity marker than raw TSS. A 200 TSS ride in two hours is a radically different stimulus to 200 TSS in five hours. The weekly total tells you volume. The per-hour figure tells you what kind of work you actually did. Track both."
  - "TSS systematically underestimates easy aerobic rides and overestimates short hard efforts because it cannot distinguish between the metabolic costs of different intensity domains. It is a useful tool with known limitations — treat it as a compass, not a sat-nav."
---
