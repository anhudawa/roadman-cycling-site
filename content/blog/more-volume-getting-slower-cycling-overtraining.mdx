---
title: 'I Doubled My Cycling Hours And Got Slower. Why That Happens.'
seoTitle: 'Doubled Training Volume, Got Slower? The Five Real Causes'
seoDescription: >-
  More training but got slower on the bike? In cycling that's a recognisable
  pattern. The five real causes, and the diagnostic sequence to fix each one.
description: >-
  A listener wrote in to the Roadman Cycling Podcast having
  doubled his weekly training volume and lost speed. Anthony
  Walsh walks through why volume without recovery, intensity
  control, periodisation, and the right testing target produces
  the opposite of progress — and what to change first.
excerpt: >-
  Six hours a week to twelve, and slower. The pattern is so
  common it has a diagnostic checklist. Stress load, intensity
  distribution, distribution mismatch with the test, no progressive
  overload, and the missing coach to spot the plateau before it
  hardens.
date: '2026-05-03'
publishDate: '2026-05-03'
updatedDate: '2026-05-03'
author: Anthony Walsh
pillar: coaching
featuredImage: /images/cycling/gravel-road-climb.jpg
tags:
  - training-volume
  - overtraining
  - cycling-plateau
  - recovery
  - intensity-distribution
  - periodisation
  - stress-load
  - dan-lorang
keywords:
  - more training got slower cycling
  - doubled training volume slower
  - cycling plateau causes
  - overtraining cycling signs
  - cycling training stress load
featuredEntities:
  - Dan Lorang
experts:
  - Anthony Walsh
  - Dan Lorang
topicTags:
  - training-volume
  - overtraining
  - intensity-distribution
answerCapsule: >-
  Doubling weekly training hours and losing speed is a
  recognisable diagnostic pattern. Anthony Walsh on the Roadman
  Cycling Podcast walks through five causes. One — total stress
  load is too high. Dan Lorang describes the stress bucket
  framing — work, family, relationships, and training all draw
  from the same recovery capacity. Two — intensity control is
  off and the rider is accumulating grey zone work that fatigues
  without driving adaptation. Three — the training distribution
  has shifted entirely to volume but progress is being measured
  against a threshold or VO2 max test, which the volume work was
  never designed to develop. Four — there is no progressive
  overload. The body adapted to the new stress and then sat
  there. Five — there is no coach to read the data and spot the
  plateau before it hardens. The fix sequence is to audit
  recovery first, then intensity distribution, then periodisation,
  then nutrition, and only then escalate to medical investigation
  with a full blood panel through the rider's GP.
keyTakeaways:
  - Volume alone produces gains up to a point. Doubling from six to twelve hours per week generally improves fitness. Doubling from twelve to twenty-four often does not — recovery capacity caps the return.
  - Dan Lorang's stress bucket framing — training, work, family, and relationships all draw from the same recovery capacity — is the diagnostic that catches most volume-induced plateaus before any training change is needed.
  - Adaptation happens during recovery, not during training. Training creates the potential. Recovery realises it. Riders who add hours without adding sleep, fuel, or rest days are training without adapting.
  - Most riders who add volume drift into the grey zone. The new hours go in at zone 3, accumulating fatigue without driving the threshold or VO2 max adaptations the rider is testing for.
  - Measuring the wrong thing makes the problem invisible. Riders who add zone 2 volume and then test FTP often see no change — the volume work was not designed to develop FTP. The test is measuring the wrong adaptation.
  - Progressive overload matters across weeks and months, not just within sessions. A static training week repeated for ten years produces one round of adaptation and then a plateau. The stimulus has to evolve.
  - The role of a coach is most visible during plateaus. Anyone can prescribe sessions. The diagnostic work — reading the data, spotting the pattern, identifying which lever to pull first — is where good coaching pays for itself.
relatedEpisodes:
  - ep-2050-trying-to-get-faster-you-might-be-making-this-mistake
  - ep-2148-80-20-training-to-ride-faster-dr-stephen-seiler
  - ep-2-5-fixable-mistakes-self-coached-cyclists-make
  - ep-2113-5-common-training-mistakes-even-pro-s-make-and-how-to-fix-th
relatedPosts:
  - 80-20-cycling-training-the-grey-zone-trap
  - polarised-training-cycling-complete-guide
  - biggest-training-mistakes-from-coaches
  - is-a-cycling-coach-worth-it
  - best-online-cycling-coach-how-to-choose
  - cycling-fatigue-signs-when-to-back-off
  - cycling-recovery-week-what-to-actually-do
faq:
  - question: Why am I getting slower after doubling my training volume?
    answer: >-
      The five most common causes, in diagnostic order, are
      excessive total stress load relative to recovery capacity,
      poor intensity distribution that pushes new volume into the
      grey zone, a mismatch between the type of training added and
      the test being used to measure progress, no progressive
      overload across weeks so the body adapts and then plateaus,
      and the absence of a coach to read the data and identify
      which lever to pull. Audit recovery first, then intensity
      distribution, then periodisation, then nutrition. If all four
      check out and the plateau persists, escalate to a full blood
      panel through the rider's GP.
  - question: What is the stress bucket and how does it apply to training?
    answer: >-
      The stress bucket is Dan Lorang's framing for total recovery
      cost. Training stress, work stress, family stress, and
      relationship stress all draw from the same recovery capacity.
      The body does not separate training stress from life stress
      when allocating recovery resources. A rider who doubles
      training volume during a high-stress work period is
      effectively asking for two large recovery debts simultaneously.
      Adaptation requires the recovery to land. The most common
      cause of a volume-induced plateau is that the bucket is full
      from non-training stress and the new training is going
      uncovered.
  - question: How do I know if my intensity distribution is the problem?
    answer: >-
      Pull the intensity distribution chart from TrainingPeaks or
      a similar platform across the last four to eight weeks. If
      most of the new volume sits in zone 3 — the grey zone, between
      solid zone 2 and threshold — the distribution is the likely
      culprit. The polarised target is roughly 80 per cent of total
      time at low intensity (zone 1 to 2), 20 per cent at high
      intensity (zone 4 to 5), and very little time in zone 3.
      Riders who add volume tend to drift into the middle by
      default because zone 2 feels too easy to count as serious
      training. The fix is to push easy days easier and hard days
      harder.
  - question: Could I be measuring the wrong adaptation?
    answer: >-
      Yes — and this is one of the most common mistakes. Adding
      large volumes of zone 2 work develops the aerobic base. It
      does not directly develop threshold or VO2 max. A rider who
      builds an aerobic block over a winter and then tests FTP
      will often see no improvement, conclude the training did
      not work, and lose confidence in the structure. The volume
      work was never designed to lift FTP in the short term. It
      builds the base that lets the spring threshold work pay off.
      Match the test to the adaptation the training was designed
      to drive.
  - question: When should I see a doctor about a training plateau?
    answer: >-
      If recovery is well-managed, intensity distribution is sound,
      periodisation is in place, nutrition is adequate across
      macronutrients and micronutrients, and the rider is still
      regressing or stuck, the next step is medical. A full blood
      panel through the rider's GP — iron, ferritin, vitamin D,
      thyroid markers, inflammatory markers, hormone profile — is
      the standard escalation. Persistent under-performance with
      no obvious training cause often has a physiological signal
      buried in the bloodwork. Anaemia, low ferritin, and
      sub-clinical thyroid issues are common in serious amateurs,
      particularly across high-volume blocks.
whoFor:
  - "Riders who added significant training hours and lost speed instead of gaining it"
  - "Self-coached cyclists stuck in the grey zone without realising their intensity distribution has drifted"
  - "Anyone measuring progress with the wrong test for the training they have been doing"
  - "Riders carrying high life stress who have not connected that to their training plateau"
roadmanView:
  - "This is one of the most common questions we get on the podcast. A rider doubles their hours, expects to fly, and gets slower. The pattern is so recognisable it has a diagnostic checklist, and the answer almost never starts with training — it starts with recovery."
  - "Dan Lorang's stress bucket concept is something I come back to constantly. He coached Jan Frodeno, Anne Haug, and Mark Cavendish, and his framing is simple: training stress, work stress, and family stress all draw from the same recovery capacity. If the bucket is already full, more training just overflows."
  - "The other one that catches people is measuring the wrong thing. You do a big base block of Zone 2 and then test your FTP. Of course it has not moved — that is not what the training was designed to do. Match the test to the adaptation. It sounds obvious, but I have seen experienced riders make this mistake for years."
---

Doubling weekly training hours and losing speed is one of the most common questions on the Roadman rider support podcast. The pattern is so familiar that Anthony Walsh and Sarah have a diagnostic sequence for it. The rider has done what every cycling article said to do — added volume — and the bike has gone slower instead of faster. The frustration in the message is always the same. The intuition that more should equal better has run into the wall of recovery capacity.

The conversation matters because the volume-doubling plateau is structurally different from a normal plateau. The rider has added the input the cycling internet usually credits as the answer. The output has gone the wrong way. The diagnostic question is not whether to train more. It is which of the five common causes is in play.

[Listen to the full episode →](/podcast/ep-2050-trying-to-get-faster-you-might-be-making-this-mistake)

This piece walks through the five causes in the order Anthony works them, the framing Dan Lorang gave on the stress bucket conversation, and the practical fix sequence.

## Volume Has A Ceiling

The first useful framing is honest about where volume helps and where it stops helping. A rider who moves from six hours a week to twelve hours a week will almost certainly improve. The ceiling on aerobic adaptation at six hours per week is well below what the average serious amateur is capable of. Doubling the input from a low base lifts the output reliably.

The same is not true at the upper end. A rider moving from twelve to twenty-four hours per week is asking for double the recovery debt at a load most amateurs cannot service alongside a professional career, family commitments, and an adequate sleep budget. The same intervention that worked at the bottom of the scale fails at the top. Recovery capacity is the limiting factor, not training capacity.

The mistake is to treat volume as a single lever that always rewards more pulling. The lever rewards more pulling up to a point, and then it punishes it. The diagnostic for which side of the line the rider is on is whether the recovery infrastructure — sleep, fuel, life stress — has scaled with the training load.

## The Stress Bucket

Dan Lorang's framing on the podcast — referenced in this Anthony and Sarah episode and developed at length in his own appearance — is the cleanest single concept for thinking about volume-induced plateaus. The stress bucket is fixed. Training stress goes in. Work stress goes in. Family stress, relationship stress, and the underlying baseline stress of normal life all go in the same bucket. The body does not separate them. The recovery resources allocated to repair are drawn from a single pool.

A rider who doubles training volume in a calm life stretch generally absorbs the new stress. The bucket has room. The recovery resources are available. The new training drives adaptation.

A rider who doubles training volume in a high-stress work period, a relationship transition, a young-children year, or any other period of elevated baseline cost is effectively requesting two large debits from a bucket that is already running near full. The body cannot service both. The training stress accumulates as fatigue rather than adaptation. The plateau follows within four to six weeks.

The diagnostic check is to audit life stress before auditing training. If the bucket is full from non-training inputs, the fix is not more or different training. The fix is to either reduce the non-training stress or scale back the training to match the available recovery capacity.

## The Grey Zone Drift

The second cause is structural. Most riders who add volume drift into the grey zone by default. The new hours go in at moderate-hard intensity — too hard to be true zone 2, not hard enough to be true threshold work. Across weeks of accumulated grey zone training, the recovery cost rises faster than the adaptation.

The mechanism is well-characterised in the polarised training research. Zone 3 work — the moderate-hard range between solid zone 2 and threshold — produces a fatigue response close to threshold work and an adaptation response closer to zone 2 work. The dose-response curve is unfavourable. The rider feels like they are training, the sessions leave them tired, and the fitness gain is thinner than the fatigue cost would predict.

The fix is the same fix the [polarised training framework](/blog/polarised-training-cycling-complete-guide) prescribes. Easy days have to be properly easy. Heart rate at 65 to 75 per cent of maximum, conversation in full sentences without stopping for breath, perceived effort around 4 out of 10. Hard days have to be properly hard. The middle ground is the trap.

The TrainingPeaks intensity distribution chart is the diagnostic tool. Pull the last four to eight weeks. If most of the volume is sitting in zone 3 rather than zone 2, the distribution is the cause — the [FTP zone calculator](/tools/ftp-zones) sets exact boundaries so you know where zone 2 ends and the grey zone begins. Push the easy work easier and the hard work harder until the chart shifts toward the polarised target. For a deeper treatment of the grey zone trap and how to escape it, see [the dedicated piece](/blog/80-20-cycling-training-the-grey-zone-trap).

## The Test-Training Mismatch

The third cause is more subtle and is missed by riders who would otherwise diagnose the volume problem cleanly. The training has been adjusted, but the test being used to measure progress has not. The rider added a winter aerobic block. The rider tests FTP in February. The FTP barely moves. The conclusion is that the training failed.

The actual problem is that the volume block was never designed to lift FTP in the short term. The aerobic base work develops mitochondrial density, capillary network, fat oxidation capacity, and the underlying engine that supports threshold work. The threshold itself moves modestly off pure aerobic work. The threshold-specific adaptation comes from the threshold-specific block that follows the aerobic base.

Anthony's framing on the podcast is direct. If you study English and then take a biology test, the test result is not a failure of the studying. It is a mismatch between the studying and the test. Aerobic volume is the right input for a test of aerobic durability — long climbs, steady-state hour-plus efforts, fatigue resistance late in long rides. A 20-minute power test is a poor measurement of an aerobic block.

The fix is to match the test to the training phase. If the training is aerobic, the test is aerobic durability — heart rate at a fixed power across a 90-minute steady ride, drift from start to finish, post-ride recovery markers. If the training is threshold-specific, the FTP test is the right measurement. If the training is VO2 max-specific, a five-minute or three-minute power test is the relevant marker. The aerobic block builds the engine. The threshold block sharpens it. The two work together. They do not substitute for each other.

## No Progressive Overload

The fourth cause is the gym analogy. A rider who lifts the same weights for the same reps twice a week for ten years and looks the same at the end of it has not failed at training. They have completed one round of adaptation and then sat there. The body absorbed the new stress, adapted to it, and then settled. Without a progressive change in the stimulus — heavier weights, more reps, different angles — the body has no reason to adapt further.

The same logic applies to cycling volume. A rider who increases from six to twelve hours per week, holds at twelve for two years, and then complains of plateaued fitness has not failed. They have completed the adaptation to twelve hours. The fix is not more time at twelve. It is a deliberate change in the stimulus — an intensity block, a periodised build phase, a structured taper, a different distribution of zones, a strength training addition.

The deeper mistake here is to interpret consistency as progression. Showing up reliably is the foundation. It is not, by itself, the work that drives ongoing improvement. The work is in the deliberate evolution of the stimulus across months and across years.

For amateurs working through how to structure a progressive plan, the [comeback cyclist 12-week return plan](/blog/comeback-cyclist-12-week-return-plan) and the [strength training guide for cyclists](/blog/cycling-gym-exercises-best) lay out two of the most common entry points.

## The Coach Question

The fifth cause is the absence of a diagnostic eye. Anthony's framing on the podcast is consistent — anyone can write a training plan. Templates are everywhere. The work that justifies a coach is the diagnostic work that happens when a plan stops working. Spotting the plateau before it hardens. Reading the intensity distribution chart and flagging the drift. Pulling the rider back when life stress is up and pushing them forward when the bucket has room. Knowing when the next test is appropriate and what test to run.

This is the part of coaching that is invisible from the outside and that matters most over multiple-year development arcs. A rider who has plateaued for six months is paying a high opportunity cost. A coach who diagnoses the cause in the first month and adjusts the structure is recovering most of that cost. The pricing of cycling coaching looks expensive against the marginal session and looks cheap against the avoided plateau.

For deeper context on whether and when to take on a coach, see [is a cycling coach worth it](/blog/is-a-cycling-coach-worth-it) and the [online cycling coach selection checklist](/blog/best-online-cycling-coach-how-to-choose). The [Roadman coaching system](/coaching) is built around the diagnostic-first model — the coaching value is in the reading of the data and the structural adjustments, not in the prescription of generic templates.

## The Fix Sequence

The practical sequence Anthony works on the podcast is the same one to run on any volume-induced plateau.

**One. Audit recovery.** Sleep, life stress, training-to-rest ratio. If recovery is the bottleneck, no training change will help until recovery is fixed. The [training load tracker](/tools/training-load) will show whether your acute load has outrun your chronic fitness — if the ratio is off, that's your answer before you change a single session.

**Two. Audit intensity distribution.** Pull the chart. Check the proportion in each zone. Push toward the 80/20 polarised structure if the chart is grey-zone heavy.

**Three. Audit periodisation.** Has the structure progressed across weeks and months, or has the same week been repeated? Add the next stimulus the body has not yet adapted to.

**Four. Audit nutrition.** Macronutrient adequacy, in-ride fuelling, micronutrient status. Under-fuelled training does not produce adaptation regardless of the structure.

**Five. Escalate to medical.** A full blood panel through the rider's GP. Iron and ferritin in particular are common deficiencies in high-volume amateurs. Thyroid markers, vitamin D, and a hormone profile complete the picture.

If all five check out and the plateau persists, the structural issue is usually one the rider has not surfaced — a chronic illness, a hidden life stress, an environmental factor. At that point the coaching conversation is no longer about training and is about the broader picture. If you want a faster triage of which lever is most likely the bottleneck, the [3-minute plateau diagnostic](/plateau) walks through the same audit sequence in question form.

## What The Roadman Audience Should Take Away

Three things from the conversation translate directly to the serious amateur.

**One. Recovery is the limit, not training.** Adding hours to a calendar that does not have the recovery capacity to absorb them produces the opposite of fitness. The first lever to pull when more training is not working is recovery, not different training.

**Two. The grey zone is the most common volume mistake.** New hours that drift into zone 3 fatigue the rider without driving adaptation. The 80/20 polarised distribution remains the structural target. Easy easier, hard harder, very little in between.

**Three. The plateau is a diagnostic problem, not a training problem.** The fix is not always more or different training. The fix is to identify which of the five causes is in play and address that specifically. Generic interventions on a misdiagnosed plateau make the problem worse.

## Listen To The Full Conversation

The full episode — including the further questions on aero positioning, masters racing, and stage race team management — is on the [Roadman Cycling Podcast](/podcast/ep-2050-trying-to-get-faster-you-might-be-making-this-mistake).

Six hours to twelve, and slower. The pattern is recognisable. The fix sequence is concrete. The serious amateur who runs the diagnostic in order, fixes the bottleneck, and resists the urge to add more of the wrong thing is the one who breaks the plateau and gets the volume to do the work it was supposed to do.
