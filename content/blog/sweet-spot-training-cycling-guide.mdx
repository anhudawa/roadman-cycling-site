...
**The progression.** Add time before you add intensity. Week to week, grow the total minutes at sweet spot — 36 minutes, then 40, then 45 — rather than nudging the watts up. The power target tracks your FTP; the volume of work at that target is the lever you actually pull, which is exactly the dose-frequency-duration logic [we worked through on the threshold episode](/podcast/ep-2540-secret-to-improving-threshold-dose-frequency-duration). Hold the same target for the block, retest, then reset.
