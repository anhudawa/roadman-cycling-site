...
**Days 1-3 after symptom clearance:** A conservative pause may be appropriate after fever or chest symptoms, but there is no universal three-day immune window. The “open window” mechanism is contested and return timing depends on the illness, severity, symptoms and medical advice. Do not train with fever or significant systemic symptoms.
...
**Heavy training blocks** can coincide with illness risk through several interacting factors, including exposure, sleep, nutrition, travel and total load. The simple claim that every hard session creates 3-72 hours of immune suppression is not established.

After illness, use symptoms and the response to light activity to guide a cautious return. A relapse or new illness is not proof that a hard session opened an immune “window”; stop and get reassessed if symptoms return or worsen.
...
Recurrent illness can coexist with high load, inadequate recovery or low energy availability, but it can also have an unrelated medical cause. Do not diagnose chronic immunosuppression from a training log.

Read the [cycling fatigue signs and back-off guide](/blog/cycling-fatigue-signs-when-to-back-off). If illness keeps returning, reduce training stress and seek clinical advice rather than assuming load is the only cause. The [comeback guide](/blog/cycling-comeback-after-break-complete-guide) and the [12-week return plan](/blog/comeback-cyclist-12-week-return-plan) both have frameworks for building back with recovery baked in.
