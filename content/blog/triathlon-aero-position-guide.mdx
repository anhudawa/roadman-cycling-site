---
title: 'Triathlon Aero Position: How to Actually Make It Faster, Not Just Look Pro'
seoTitle: 'Triathlon Aero Position: How to Make It Actually Faster'
seoDescription: >-
  Your triathlon aero position won't get faster by slamming the pads. Set up one
  you can hold, fuel, and run off, that works on the clock, not just in photos.
excerpt: >-
  Slamming the pads and going low doesn't make you faster. A position you can
  hold, fuel and run off does. Here's how to actually make your triathlon aero
  position faster on the clock, not just in photos.
pillar: coaching
author: Anthony Walsh
updatedDate: '2026-04-23'
publishDate: '2026-04-14'
featuredImage: /images/cycling/gravel-desert-road-epic.jpg
keywords:
  - triathlon aero position
  - tri bike fit
  - cda triathlon
  - tt position power loss
  - aero tuck triathlon
  - triathlon bike fit
  - aero vs power tradeoff
faq:
  - question: How low should my aero position be?
    answer: >-
      As low as you can hold for the full race duration without losing more than
      3-5 per cent of your seated FTP, and without cooking your hip flexors for
      the run. An aggressive position that costs 20 watts and 30 seconds off the
      run is slower overall than a moderate one you can own.
  - question: What's the biggest mistake triathletes make with aero position?
    answer: >-
      Copying a pro's position without a pro's flexibility, durability or
      training volume. A slammed front end looks fast in the race photos and
      costs 2 minutes on the run. Fit the position to the athlete, not the
      athlete to a photo.
  - question: When does a bike fit pay for itself?
    answer: >-
      Almost always, for a triathlete. A quality tri-specific fit typically
      reduces CdA by 0.02-0.05 and reduces power loss relative to the road bike.
      At 70.3 and Ironman distances that's commonly 2-6 minutes of savings —
      more than most training blocks deliver.
  - question: Does an aero position wreck the run?
    answer: >-
      A bad one does. Shortened hip flexors, closed hip angles and overloaded
      lower back all compound across the bike leg and show up as a stiff, slow
      first 5km of the run. A well-fitted position should leave you able to run
      at target pace off T2.
answerCapsule: >-
  A faster triathlon aero position is one that lowers CdA without costing
  meaningful power or destroying your run. Target pad drop you can hold for race
  duration, power loss under 3-5 per cent vs seated FTP, and hip angles that
  protect your hip flexors. A fit typically saves 2-6 minutes — bigger than most
  training blocks.
keyTakeaways:
  - "The fastest aero position is the one you can hold, fuel from, and run off — not the lowest one."
  - "A moderate TT position costs 2-4% of seated power; an aggressive one costs 4-7% and carries significant run penalty if not trained."
  - "A quality tri-specific bike fit typically saves 2-6 minutes over a 70.3 through CdA reduction and reclaimed power — larger than most training blocks."
  - "Hip angle below 90 degrees strangles power and destroys hip flexors for the run; daily mobility work (pigeon, 90/90) is non-negotiable."
  - "You earn an aggressive position by spending 6-10 hours a week in it across months — it cannot be bought at a fit studio two weeks out."
relatedEpisodes:
  - ep-2165-triathlete-improve-your-swim-in-your-30s-and-40s-josh-amberg
  - ep-2161-how-to-create-the-perfect-triathlon-training-plan-olav-bu
  - ep-2027-train-slower-ride-faster-why-it-actually-works
relatedPosts:
  - aero-position-training-for-triathletes
  - triathlon-cycling-power-to-weight
  - aero-for-amateur-cyclists-does-it-matter
  - bike-fit-guide-cyclists
  - triathlon-cycling-training-plan
whoFor:
  - Triathletes who have copied a pro position and paid for it on the run
  - 70.3 and Ironman age-groupers looking to lower CdA without losing power or wrecking T2
  - Riders considering a tri-specific bike fit and wanting to know what it should deliver
  - Anyone who has gone low and narrow for the race photo and then run 10 minutes slower
roadmanView:
  - A slammed front end that costs you 20 watts and 30 seconds on the run is slower overall than a moderate position you can own for the full distance. Fit the position to the athlete, not the athlete to a photo.
  - The aero position that makes you faster is the one you can hold, fuel from and run off. If you cannot do all three, the position is wrong regardless of what it looks like.
  - A quality tri-specific fit typically reduces CdA by 0.02 to 0.05. At Ironman distance that is commonly two to six minutes of savings, more than most training blocks deliver.
---

Every triathlete knows aero matters. Most of them get the execution wrong. They see a pro slammed low and narrow in a magazine photo, drop their own pads 20mm, roll their shoulders in, and proudly go 8 watts lower off the bike while their hip flexors scream from kilometre 2 of the run.

Aero isn't about looking pro. It's about finding the position that minimises the product of two numbers: the air you push, and the time you bleed on the run. Get either one wrong and you're slower overall, even if the race photo is beautiful.

We've had some of the fastest riders in cycling and some of the best bike splits in triathlon on the podcast, and they all say the same thing: the fastest position is the one you can hold, fuel from, and run off. Not the lowest one.

## The Trade-Off Most Triathletes Get Wrong

Here's the equation every triathlete should have burned into their head:

**Total race time = bike time + run time + (position penalty on run).**

A more aggressive aero position usually does reduce bike time. It also usually reduces seated power output (you're breathing worse and recruiting muscles worse), and usually increases run penalty (tight hip flexors, compressed diaphragm, stiff lower back).

Typical numbers from real bike fits and wind tunnel testing:

- **Moderate TT position:** CdA 0.27-0.30, power loss vs road seated of 2-4 per cent, minimal run penalty.
- **Aggressive TT position:** CdA 0.23-0.26, power loss 4-7 per cent, significant run penalty if not trained.
- **Extreme pro position:** CdA 0.20-0.23, power loss 6-12 per cent, huge run penalty for amateurs.

The amateur mistake is copying the third position while training for the first. You lose more on the run than you gained on the bike, and the whole exercise costs you minutes.

The right answer is position-specific training. You earn an aggressive position by spending 6-10 hours a week in it across months. You don't buy it at a fit studio two weeks out.

## Key Position Metrics Worth Knowing

A triathlete with a half-decent understanding of the variables fitters work with can have a far more productive conversation with a fitter and make better decisions. The core numbers:

Before you start changing pad height, make sure you actually know your seated FTP — those numbers are the baseline you're testing position against, and the [Roadman FTP zones calculator](/tools/ftp-zones) is the quick way to set them honestly.

**Pad drop.** The vertical distance from saddle to arm pads. More drop = more aero, more closed hip angle, more run cost. Age-groupers typically live around 10-20cm; pros can push 25cm+.

**Hip angle.** The angle between torso and thighs at the top of the pedal stroke. Closed hip angles (below ~90°) strangle power and close the hip flexors. A good fit finds the drop-hip-angle trade-off for your specific mobility.

**Arm pad width.** Narrower is aero up to a point, but beyond that you start losing power and stability. Elbow-width is usually the cheapest aero gain you haven't made.

**Saddle position.** Tri saddles typically sit further forward than road — 76-78° effective seat angle — to open the hip angle at a lower torso. This is non-negotiable for a good TT fit.

**CdA.** The outcome metric. If you can't test it (tunnel, velodrome, Aerolab/Notio, chung-style field test), at least track your average speed at known power on the same flat route repeatedly. It's a noisy proxy but it beats nothing.

## When a Fit Pays for Itself

A good tri-specific bike fit costs $200-500 in the UK/Ireland and typically takes 3-6 hours. It's the highest-ROI spend in triathlon outside coaching.

Typical savings from a first-time quality tri fit for an age-grouper:

- **CdA reduction:** 0.03-0.06. At 40 kph that's 8-20 watts of drag saved, or roughly 1.5-4 minutes over a 90km bike leg.
- **Power reclaimed:** 2-4 per cent more seated power from better hip and knee alignment.
- **Run cost reduced:** Less hip flexor tightness and lower back fatigue, often worth 30-90 seconds on the half marathon.

Total savings from a fit on a 70.3: routinely 2-6 minutes. That is bigger than any 8-week training block most amateurs do. Go get fitted.

The one caveat: get fitted by someone who actually understands tri fit, not a road fitter using the same protocol. Ideally someone with wind tunnel or CFD experience, or at minimum a fitter who uses a real-world aero test as part of the process.

## Hip Flexors and the Cost on the Run

Every minute you spend in the aero bars is a minute your hip flexors are contracted. Over a 5-hour Ironman bike that's 5 hours of shortened iliopsoas, tight TFL, and a pelvis locked forward. Then T2 asks those same muscles to lift your knees 10,000 times in an hour.

The run penalty from a bad position shows up as:
- A slow, stiff first 3-5 km until you "unglue" from bike position.
- A chronic shuffle on the back half of the marathon when glycogen dips and form collapses.
- Lower back pain from hour 2 of the run.

The fix is threefold: a position that doesn't crush the hip angle, consistent off-bike mobility work, and brick training to teach the body to transition. If you're not doing daily hip mobility as a triathlete, the position you can hold and run off is more restricted than it needs to be. Yoga, pigeon variations, 90/90 work — 10 minutes a day goes a long way.

## What To Do This Month

1. **Photograph your current position.** Side view, on the trainer in full TT. Measure pad drop and hip angle.
2. **Do a flat, steady CdA test.** Same road, same power, same kit, compare speed.
3. **Get a tri-specific bike fit** if you haven't had one in the last 12 months or if your fit was done by a road-only fitter.
4. **Spend more time in the bars in training.** At least 60-70 per cent of bike training volume in race position. Position adaptation takes weeks.
5. **Pair position work with pacing discipline** — see [triathlon bike pacing](/blog/triathlon-ftp-pacing-strategy). Holding an aggressive position at threshold costs triple what it costs at endurance.
6. **Build the engine underneath it** via [a triathlon-specific training plan](/blog/triathlon-cycling-training-plan). You cannot fake the fitness your position demands.

## Key Takeaways

- The fastest aero position is the one you can hold, fuel from and run off — not the lowest.
- Moderate TT positions cost 2-4 per cent of seated power; aggressive ones cost 4-7 per cent; extreme pro positions cost much more.
- A good tri fit typically saves 2-6 minutes over 70.3 via CdA reduction and reclaimed power — larger than most training blocks.
- Track pad drop, hip angle, arm pad width and effective seat angle — these are the levers.
- Protect your hip flexors with daily mobility work and position-specific training volume.
- Earn an aggressive position by living in it for months, not by slamming pads at the fit studio.
- Pair your position with [pacing discipline](/blog/triathlon-ftp-pacing-strategy) and [a cycling-specialist training plan](/blog/triathlon-cycling-training-plan).
- Want this dialled in properly before your A race? See our [coaching options](/coaching) or [apply to work with us](/apply).
- For an engineering-led view on amateur aero gains, see [Dan Bigham on aerodynamics for amateur cyclists](/blog/dan-bigham-aerodynamics-amateur-cyclists)
- The pro-team protocols Matt Bottrill uses translate down — see [Matt Bottrill's 7 pro hacks](/blog/matt-bottrill-7-pro-hacks)
