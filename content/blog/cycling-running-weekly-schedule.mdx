...
whoFor:
  - "Cyclists who want to add running but keep bolting runs on top of a full programme and burning out by Thursday"
  - "Riders with eight to twelve hours a week who need a template that sequences both sports sensibly"
  - "Off-season cyclists looking for a practical cross-training schedule that protects key bike sessions"
  - "Duathletes or multisport-curious riders building their first combined weekly plan"
roadmanView:
  - "The wrong way to add running is to pile it on top of everything else. The right way is to swap out the low-value cycling sessions -- the junk miles, the easy spins -- and put runs there instead."
  - "Always cycle first and run second on double days. Cycling is concentric and non-impact, so it does not compound the eccentric damage that running produces when you reverse the order."
  - "Running is always the first session I cut when training load needs to drop. It is supplementary. The bike is primary. If something has to go, the run goes."
---
...
Running replaces the junk. A 25-minute easy run delivers a more productive aerobic stimulus than a 45-minute easy spin, takes less total time including preparation, and adds the [bone density](/blog/cycling-bone-density-running-fix) and neuromuscular benefits that cycling cannot provide. The [run-ride converter](/tools/run-ride-converter) helps you swap sessions by equivalent effort rather than guessing. The total training load stays similar; the composition changes.
...
**HRV (Heart Rate Variability).** If you use a Whoop, Garmin, Oura, or similar device that measures HRV, watch for declining trends over a rolling seven-day window. The [masters recovery score](/tools/masters-recovery-score) pulls together several of these signals into a single readiness check. HRV is more sensitive than resting heart rate to accumulated training stress and can flag overreaching before symptoms appear. The absolute HRV number matters less than the trend direction — a consistent downward slope over a week signals that the body is not absorbing the current training load.
