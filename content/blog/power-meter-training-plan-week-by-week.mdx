---
title: 'Power Meter Training Plan: A Week-by-Week Guide'
seoTitle: 'Power Meter Training Plan: A Week-by-Week Guide'
seoDescription: >-
  A week-by-week power meter training plan that teaches you to use watts for
  real training decisions — zones, session targets, TSS management, recovery.
excerpt: >-
  You have a power meter. Great. Now what? Here's the week-by-week plan that
  teaches you to train by watts — zones, session targets, TSS management, and
  the mistakes new power-meter owners always make.
pillar: coaching
author: Anthony Walsh
updatedDate: '2026-04-23'
publishDate: '2026-04-17'
featuredImage: /images/cycling/gravel-road-climb.jpg
keywords:
  - power meter training plan
  - cycling power meter training
  - training with power meter
  - watts training plan
  - power based training
  - FTP test
  - TSS cycling
answerCapsule: >-
  A power meter training plan uses watts rather than heart rate or perceived
  effort to set session targets. The process is four steps: establish FTP with a
  20-minute or ramp test, set seven training zones from that FTP, follow an
  8-week progression that builds volume before intensity, and monitor weekly TSS
  between 300 and 600 depending on training age. Done correctly, power training
  removes guesswork from pacing, intervals, and recovery decisions within six to
  eight weeks.
keyTakeaways:
  - "Establish FTP with a 20-minute test (take 95% of average power) or a ramp test, and retest every 6-8 weeks during a build block."
  - "Zone 2 for volume and Zone 4 for intensity drive 90% of fitness change in a typical amateur — master those before worrying about the other five zones."
  - "The 8-week progression builds from aerobic foundation (TSS 350-450) through threshold work to VO2max intervals (TSS 500-600), with a recovery week at week 5."
  - "Weekly TSS should rise no more than 5-8% per week; a drop of more than 8-10% in morning HRV from your rolling baseline signals accumulated fatigue."
  - "The most common mistake is spending all week in Zone 3 — it feels productive but produces moderate fatigue for moderate adaptation."
  - "Train by power, monitor with heart rate: rising HR at the same power signals fatigue, falling HR at the same power signals fitness gains."
faq:
  - question: How long does it take to see results from power meter training?
    answer: >-
      Most riders see measurable FTP gains of 5-15% within 8-12 weeks of
      structured power-based training, assuming they started from unstructured
      riding. The first gains come from better pacing and executing intervals at
      the right intensity rather than physiological change. Genuine adaptation —
      mitochondrial density, lactate clearance — takes 6-8 weeks of consistent
      work before it shows up in test numbers.
  - question: Do I need to do an FTP test every month?
    answer: >-
      No. Testing every 6-8 weeks is enough for most riders. Testing too often
      wastes a hard session and the number barely moves week to week anyway. Use
      field indicators instead — how a 2x20 at threshold feels, whether Zone 2
      power at the same heart rate is climbing. Formal retesting confirms what
      training data already suggests.
  - question: What's the difference between TSS and training load?
    answer: >-
      TSS (Training Stress Score) measures the stress of a single session
      relative to your FTP. A 1-hour ride at FTP equals 100 TSS. Training load
      is the rolling average of TSS over time, usually 7 or 42 days. TSS is the
      input; training load is what your body actually adapts to. Managing weekly
      TSS prevents the spikes that cause injury and burnout.
  - question: Can I use a power meter training plan without a coach?
    answer: >-
      Yes, for a while. A structured 8-12 week block executed honestly will
      improve most self-coached riders. The limit comes when progress stalls and
      you need someone to interpret why — fatigue masking fitness, zones
      drifting, life stress distorting data. That's usually the 6-9 month mark,
      when the decisions get harder than the sessions.
  - question: Should I train by power or heart rate?
    answer: >-
      Train by power, monitor with heart rate. Power tells you the work you're
      doing right now; heart rate tells you the cost. Together they reveal
      fatigue, heat stress, and fitness changes that either metric alone hides.
      A rising heart rate at the same power across a block signals accumulated
      fatigue. Falling heart rate at the same power signals fitness gains.
relatedEpisodes:
  - ep-2032-i-cut-my-training-in-half-here-s-what-no-one-tells-you
  - ep-2094-5-crucial-winter-training-tips-i-wish-i-knew-sooner-rider-su
  - ep-2117-dylan-johnson-opens-up-about-weird-unbound-preparation
experts:
  - name: Dan Bigham
    role: Former Hour Record holder; aerodynamics engineer
    href: /guests/dan-bigham
  - name: Prof. Stephen Seiler
    role: Sports scientist; polarised-training researcher
    href: /guests/stephen-seiler
  - name: Dan Lorang
    role: 'Head of Performance, Red Bull–Bora-Hansgrohe'
    href: /guests/dan-lorang
relatedPosts:
  - how-to-improve-ftp-cycling
  - power-meter-buying-guide-cyclists
  - power-meter-training-cyclists-how-to-use
  - power-meter-buying-guide-cyclists
  - cycling-metrics-explained
whoFor:
  - "New power-meter owners who want a structured week-by-week plan to learn training by watts"
  - "Self-coached cyclists currently riding without a plan who want to build from aerobic base through threshold to VO2max"
  - "Riders stuck in the Zone 3 grey zone who need clear guidance on intensity distribution"
  - "Athletes managing training load for the first time who want TSS targets and recovery-week structure"
roadmanView:
  - "I say this to every rider I coach: Zone 2 and Zone 4 drive 90 per cent of fitness change in a typical amateur. Master those two before you worry about the other five — and definitely before you start hammering Zone 5 work you're not ready for."
  - "The most common mistake I see is riders spending the whole week in Zone 3. It feels productive, but it produces moderate fatigue for moderate adaptation. That's a bad trade, and I've watched it stall people for months."
  - "I always point riders to the Roadman FTP Zone Calculator to set zones from a recent test, then follow the 8-week progression here. If your morning HRV drops more than 8-10 per cent from your rolling baseline, your body is telling you to back off — listen to it."
---

You bought a power meter. It's on the bike. The numbers are flashing on your head unit every pedal stroke. And most of the time you're still riding the way you rode before — by feel, by the group, by whatever the Strava segment demands.

That's the gap this article closes. A power meter is a measuring instrument, not a training plan. Until you translate the numbers into decisions — what to do in the next interval, whether to ride tomorrow, when to back off — the device is expensive jewellery.

What follows is the week-by-week structure we use with athletes on [our coaching](/coaching) programme when they first go power-based. Eight weeks, concrete targets, and the specific mistakes that waste the first three months for almost everyone.

## What a power meter does that heart rate can't

Heart rate lags. Ride for 30 seconds at 400 watts and your heart rate is still climbing into zone when the effort is already over. That lag makes heart rate useless for intervals under about five minutes and unreliable for anything above threshold.

Power is instantaneous. The watts you produce in the current pedal stroke show on the screen in the current pedal stroke. That means you can pace a 30-second effort, a 3-minute VO2 max interval, and a 20-minute threshold block with the same tool and identical precision.

Power is also honest. Heart rate drifts with heat, dehydration, caffeine, sleep, and stress. If you did a 2x20 at threshold heart rate last Tuesday and again this Tuesday, the work performed could differ by 15%. At threshold watts, the work is identical by definition. That's why every World Tour team prescribes intervals in watts and uses heart rate as a secondary monitoring signal.

[Dan Bigham](/guests/dan-bigham) — former UCI Hour Record holder and one of the sharpest analytical minds in the sport — has said repeatedly on the Roadman Cycling Podcast that the power meter's real value is not the number itself but the ability to compare sessions over time. Same course, same power, different heart rate. That comparison is where training decisions actually come from.

The rider who uses a power meter as a live dashboard during the ride gets some benefit. The rider who uses it as an analysis tool after the ride gets most of it.

## Step 1: Establish your FTP honestly

Everything downstream depends on this number. FTP is the highest power you can sustain for roughly an hour. Get it wrong and every zone is wrong, every interval is wrong, every TSS calculation is wrong.

Two reliable tests. The 20-minute test: 15 minutes easy, then 5 minutes all-out to open the legs, 10 minutes easy, then 20 minutes at the hardest sustainable pace you can hold without blowing up. Take 95% of your 20-minute average as your FTP.

The ramp test: a structured protocol in TrainerRoad, Zwift, or your head unit where power increases by a fixed amount each minute until you fail. FTP is calculated as 75% of your one-minute peak. This test is shorter and less psychologically brutal, but it tends to overestimate FTP for riders with strong anaerobic systems and underestimate for strong aerobic riders.

The single most common mistake: pacing the 20-minute test like a race. Riders go out at 110% of their likely FTP, blow up at minute 12, and the average ends up lower than their real threshold. Start the test at what you think you can hold for 20 minutes, not 5. If the last minute feels manageable, you paced it right.

Retest every 6-8 weeks during a build block. More often than that and you're just testing how fresh you are on the day.

## Step 2: Learn your zones (and what each one trains)

Once FTP is set, the zones derive from it. The standard seven-zone model maps onto distinct physiological adaptations.

Zone 1 (under 55% FTP) is active recovery. Zone 2 (56-75%) is aerobic base — mitochondrial density, fat oxidation, capillary development. Zone 3 (76-90%) is tempo, useful but often overused. Zone 4 (91-105%) is threshold — lactate clearance. Zone 5 (106-120%) is VO2 max. Zone 6 (121-150%) is anaerobic capacity. Zone 7 is neuromuscular sprint power.

Plug your FTP into the [FTP zones calculator](/tools/ftp-zones) and write the numbers down. Print them, tape them to the top tube if you have to. For the next eight weeks every structured session has a zone target and you need to know the wattage range without thinking.

Prof. [Stephen Seiler](/guests/stephen-seiler)'s research on polarised training — discussed at length when he was on the podcast — shows that elite endurance athletes spend roughly 80% of their time in Zone 1-2 and 20% in Zone 4 and above, with very little in the middle. The temptation for self-coached riders is to spend all week in Zone 3. It feels productive. It produces moderate fatigue for moderate adaptation. Avoid it.

The two zones that drive 90% of fitness change in a typical amateur: Zone 2 for volume, Zone 4 for intensity. Master those before worrying about the rest.

## Step 3: The 8-week progression plan

Weeks 1-2 are aerobic foundation. Four rides per week: two Zone 2 rides of 60-90 minutes, one longer Zone 2 ride of 2-3 hours at weekends, one session with 3x8 minutes at 88-92% FTP (low threshold) mid-week. Total TSS target: 350-450 per week. The goal is to get comfortable pacing Zone 2 by watts without drifting into Zone 3.

Weeks 3-4 introduce real threshold work. Same structure, but the mid-week session becomes 2x20 minutes at 95-100% FTP with 10 minutes easy between. Weekend long ride extends to 3-4 hours. TSS climbs to 450-550.

Week 5 is a recovery week. Cut volume by 40%. Keep one short opener session with a few 1-minute efforts at Zone 5 to maintain sharpness. TSS drops to 250-300. Resist the urge to skip this week. It's where adaptation actually happens.

Weeks 6-7 add VO2 max. Two quality sessions per week now: one threshold (2x20 or 3x15 at 95-100%), one VO2 max (5x3 minutes at 110-118% FTP with 3 minutes easy). Long ride holds at 3-4 hours. TSS 500-600.

Week 8 retests. Full rest day Monday, opener Tuesday, rest Wednesday, FTP test Thursday. Compare to week 0. If the number hasn't moved, the issue is almost always one of three things: zones were set too high to begin with, recovery weeks were skipped, or weekly TSS exceeded what your life could absorb.

When you're running these sessions on the turbo, [TrainingPeaks Virtual](https://www.trainingpeaks.com/virtual/) is the cleanest way to execute the week's prescription — the planned workout syncs across with the targets already set, so you ride the 2x20 or the 5x3 exactly as written instead of fiddling with intervals on the fly, and the file feeds straight back into your training load.

Repeat the block with the new FTP. Most riders see 5-12% improvement in the first full cycle.

## Step 4: Reading TSS and recovery

TSS ([Training Stress Score](/answers/what-is-tss)) quantifies the stress of a session. An hour at FTP equals 100 TSS. A 3-hour Zone 2 ride is roughly 150-180 TSS. A hard interval day might hit 110-140.

Weekly TSS is the number that matters most. For riders training 6-8 hours per week, 350-500 is typical. For 10-12 hours, 500-700. For full-time amateurs at 15+ hours, 800-1000. What matters isn't the absolute number but the trend — rising by no more than 5-8% per week, with a recovery week every third or fourth week where TSS drops 30-40%.

[Dan Lorang](/guests/dan-lorang), who coached Jan Frodeno to three Ironman World Championship titles (2015, 2016 and 2019) and now heads performance at Red Bull–Bora–Hansgrohe (announced April 2026 to depart at the end of the 2026 season), has made a consistent point across his podcast appearances: the best athletes are not the ones who do the most work, but the ones who absorb the most work. Training load that you can't recover from is not training.

Two metrics tell you if you're absorbing it. First, morning heart rate variability (HRV) — a drop of more than 8-10% from your rolling baseline signals accumulated fatigue. Second, power at a given heart rate in Zone 2. If your Zone 2 ride at 140 bpm produces 20 watts less this week than last, you're cooked. Rest before the plan tells you to.

For triathletes, TSS management gets more complex because the run steals recovery from the bike. That's why triathlon bike coaching requires capping bike load specifically to protect run training, not maximising it in isolation.

## Common power-meter-owner mistakes

Chasing numbers every ride. Not every ride is a test. Zone 2 rides are supposed to feel easy. If you're hunting a new 20-minute peak on a Tuesday recovery spin, you don't understand the tool yet.

Setting FTP by ego rather than test. Riders inflate their FTP because a round number looks better. Then every interval session turns into a failed session, confidence drops, and the power meter gets blamed. The fix: test properly, accept the number, train the number.

Ignoring the 80/20 split. Too much Zone 3, not enough Zone 2, not enough Zone 4. This is the default failure mode of self-coached riders with power meters. The data gives them feedback that moderately hard efforts are producing moderate TSS, which feels like progress. It isn't.

Comparing raw watts to other riders. Power-to-weight matters for climbing. Absolute watts matter for flat time trials. A 280-watt FTP at 65kg produces more vertical speed than a 320-watt FTP at 85kg. Compare yourself to yourself over time, not to the guy on Strava.

Not looking at the data after the ride. The head unit is the cockpit. The analysis software — TrainingPeaks, intervals.icu, Garmin Connect — is where the learning happens. Spend 10 minutes after every quality session reviewing whether you actually hit the target zone, where you drifted, and what the heart rate response tells you about fatigue.

The next step is simple: test your FTP this week, set your zones from that number, and start week 1 of the 8-week progression on Monday. If you want the plan built around your specific events, schedule, and power profile rather than a generic template, that's what [NDY coaching at Roadman](/coaching) is for. [The application](/apply) is where the conversation starts.

Companion reads: [cycling power meter guide](/blog/power-meter-buying-guide-cyclists), [power meter vs smart trainer](/blog/power-meter-vs-smart-trainer), [FTP training zones complete guide](/blog/ftp-training-zones-cycling-complete-guide), [how to improve FTP](/blog/how-to-improve-ftp-cycling), and [age group FTP benchmarks 2026](/blog/age-group-ftp-benchmarks-2026).

Got a specific question — your own session selection, what to change after 8 weeks, when to retest? [Ask Roadman](/ask) for an answer drawn from the actual coach conversations on the podcast.
