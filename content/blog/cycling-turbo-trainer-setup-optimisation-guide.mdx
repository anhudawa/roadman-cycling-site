...
answerCapsule: >-
  Indoor bike fit should differ slightly from outdoor: drop saddle height by
  2-3 mm and add a slight front-end rise to counter the fixed position that
  concentrates pressure points. Calibrate the trainer before every structured
  session after a 10-minute warm-up, as temperature affects resistance by
  5-10 watts. Power discrepancies between trainer and power meter can exceed
  5-10% — dual-record to find the offset and pick one source consistently.
  A sweat guard is essential to prevent corrosion of headset bearings and
  frame within a single winter.
keyTakeaways:
