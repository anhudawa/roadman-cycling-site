...
answerCapsule: >-
  ...
  recreational, 3.3 W/kg good recreational, 3.8 W/kg strong amateur (cat 3/4),
  4.2 W/kg cat 2, 4.7 W/kg cat 1 / regional elite, and 5.2+ W/kg national /
  elite-amateur. For women, subtract roughly 0.5 W/kg at each band. Age decline
  is only ~5% per decade in trained masters athletes versus ~10% in sedentary
  adults — slower with training, not zero. Meaningful year-on-year FTP
  progression for trained cyclists is 5–15%; anything dramatically outside that
  range is usually measurement noise.
relatedEpisodes:
