...
If your zones haven't been updated from a recent FTP test, run an FTP test and reset them via the [FTP zone calculator](/tools/ftp-zones). If you're a masters cyclist, the [masters FTP benchmark](/tools/masters-ftp-benchmark) puts your number in age-grouped context. For watts per kilo calculations, the [W/kg calculator](/tools/wkg) handles the math.

If your training data has felt disconnected from the actual sessions, run the [training diagnostic](/diagnostic) — it'll identify whether the gap is in zone setting, session execution, or the type of training stimulus you're applying. For deeper coaching support around power-based training, the [coaching pathways](/coaching) cover specific options. The [Not Done Yet community](https://www.skool.com/roadmancycling) is the lower-investment route with weekly calls where power-based questions come up regularly.
