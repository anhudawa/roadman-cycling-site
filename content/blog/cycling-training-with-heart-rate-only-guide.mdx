...
answerCapsule: >-
  Set heart rate zones from a 30-minute field test, not the 220-minus-age
  formula — take the average HR from the final 20 minutes as your lactate
  threshold heart rate and anchor all five zones to it. Heart rate lag makes
  it unreliable for short VO2max intervals but perfectly usable for endurance,
  tempo, and threshold efforts over 8 minutes. Track fitness by monitoring
  resting heart rate trends, HR at a fixed speed, and recovery heart rate.
  When HR and legs disagree, RPE should override the number on the strap.
keyTakeaways:
