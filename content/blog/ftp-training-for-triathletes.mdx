...
keywords:
  ...
  - ironman ftp
answerCapsule: >-
  FTP training for triathletes follows the same physiological principles as cyclist FTP training but requires three specific modifications: lower weekly bike volume to preserve run durability, longer recovery windows because cumulative fatigue from three disciplines compounds, and deliberate session placement that protects key run days. Triathletes typically need 2 quality bike sessions per week rather than the 3-4 a cyclist can absorb, and FTP testing should be scheduled when run load is temporarily reduced to get an accurate reading.
faq:
  - question: "How often should triathletes do FTP work?"
    answer: >-
      Most triathletes handle two quality FTP-focused bike sessions per week, compared with three or four for a pure cyclist. The third discipline eats into recovery. In peak Ironman build phases, one threshold session plus one sweet spot session works well. During base, a single weekly FTP session is often enough to maintain and slowly build power without compromising run frequency.
  - question: "Is FTP still relevant for long-course triathlon?"
    answer: >-
      Yes. FTP sets the ceiling for your Ironman and 70.3 bike pace, which typically sits at 68-78% of FTP. Raising FTP by 20 watts moves your race-pace ceiling up proportionally. Dan Lorang's long-course athletes train FTP year-round, but they pair it with extensive sub-threshold volume. The mistake is chasing FTP alone without building the aerobic base that lets you hold 75% of it for five hours.
  - question: "Should I test FTP the same way as a cyclist?"
    answer: >-
      The protocol is the same — typically a 20-minute test with 95% of average power taken as FTP, or a ramp test. What changes is timing. Test during a lower run-volume week so leg fatigue doesn't suppress the number. Testing 48 hours after a long run will underestimate your true FTP by 5-15 watts. Use our FTP zones calculator to set training zones from the result.
  - question: "Why does my FTP drop when I add run volume?"
    answer: >-
      It usually doesn't drop — your ability to express it does. Running creates eccentric muscle damage and central fatigue that suppress peak bike power for 24-72 hours. Prof. Stephen Seiler's work on concurrent training shows the interference effect is real but manageable with session spacing. The fix is scheduling, not cutting bike intensity. Separate hard runs from threshold bike sessions by at least 36 hours.
  - question: "Can I raise FTP during Ironman build phases?"
    answer: >-
      Yes, but expect slower gains than a pure cyclist would see. Most triathletes build 3-8% in a 12-week block when they also protect run load. The bigger win in build phase is raising sustainable race percentage — holding 76% of FTP for the bike leg instead of 72% — rather than chasing a headline FTP number that won't transfer to a strong marathon off the bike.
---
