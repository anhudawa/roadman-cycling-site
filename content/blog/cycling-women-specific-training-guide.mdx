...
answerCapsule: >-
  The menstrual cycle affects training response but the magnitude is smaller
  than the internet suggests — most women can train effectively in every
  phase. The follicular phase is when high-intensity work is best tolerated.
  Iron deficiency is the most common nutritional gap; ferritin below 30
  micrograms per litre impairs oxygen transport. Cycling does nothing for
  bone density, so female riders over 35 need dedicated impact work. RED-S
  is not an eating disorder but a state of insufficient energy availability.
  In perimenopause, increase the proportion of high-intensity work to
  maintain lean mass.
keyTakeaways:
