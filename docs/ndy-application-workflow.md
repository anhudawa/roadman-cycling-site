...
- The Beehiiv enrollment marker is saved before the send request. If a successful response omits its journey ID, the adapter confirms the subscriber through the documented journey list with bounded polling and never repeats enrollment. A timeout, server error, interrupted request or still-unverifiable response is held as **Check Beehiiv before resending** and is never retried blindly.
...
4. Set `NDY_APPLICATION_FLOW_ENABLED=true` immediately before the verified production deployment. Set it back to `false` to restore the previous application response during rollback.
...
One hundred and sixteen focused tests pass across 12 files, including API routing, Inner Circle isolation, new/existing subscribers, opt-outs, active-new-subscriber enrollment, successful empty-response confirmation, ambiguous enrollment, duplicate questions, Anthony alert routing, purchase-state preservation and personal-link privacy. TypeScript, targeted lint, the 56 publication-gate tests, the Coral allowlist and diff whitespace checks pass. The internal-link audit found zero broken destinations, and the full production build compiled and typechecked 4,466 pages.

The production schema was inspected read-only and the additive table is present. Controlled applications 217–220 remain visibly labelled in admin. Application 219 exercised personalized email delivery, both tracked actions, Sarah Review persistence, CRM ownership, one Anthony alert with applicant Reply-To and checkout-interest tracking. Application 220 verified the provider empty-response recovery: one enrollment produced one completed journey, both provider IDs were saved and one personalized inbox message arrived. The received message was reviewed at 375px and 1200px under the Mac's dark system preference. Beehiiv emitted HTML-only MIME because its automation editor provides no separate plain-text control. The live Skool checkout showed a seven-day free trial followed by $195 USD/month plus VAT; no payment was initiated.
...
- [Not Done Yet on Skool](https://www.skool.com/roadmancycling/about): programme destination. Authenticated checkout verification confirmed the seven-day trial and $195 USD monthly price before tax; verification stopped before purchase.
