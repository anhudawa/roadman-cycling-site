...
Ordinary replies reach Anthony through Reply-To, while the public next-step page retains Sarah’s direct email address. Email-inbox replies do **not** automatically enter the website database; the **Ask Sarah** button and form provide the logged path into Sarah’s admin queue.
...
`/admin/applications/followups` shows the latest 100 workflow entries, enrollment/failure state, questions, Anthony inbox-alert state and checkout interest. It requires the existing admin authentication. The main application board has a **Sarah Review** column and a link to this view. Applicants in that column are assigned to Sarah, and detailed questions persist in the view even if CRM mirroring was unavailable at application time.
