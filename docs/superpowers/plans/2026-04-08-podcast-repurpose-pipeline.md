...
Respond with ONLY a JSON object in this exact format (no markdown, no code blocks):
{
  "twitter": {
    "tweets": [
      ...
      {"text": "Full episode: https://roadmancycling.com/podcast/${episode.slug} üéôÔ∏è", "index": 7}
    ]
  },
  "instagram": {
...
export async function fetchGuestImage(
  ...
  if (guestName) {
    console.log(`   üîç Searching for image of ${guestName}...`);
...
export function writeRepurposedContent(
  ...
  if (dryRun) {
    console.log(`\n   üì$ Would write to: ${path.relative(process.cwd(), outputDir)}/`);
...
async function processEpisode(
  ...
  console.log(`\n   üì∫ ${input.title}`);
  console.log(`   üîñ ${input.pillar} | ${input.type}${input.guest ? ` | Guest: ${input.guest}` : ""}`);
  ...
  if (!skipQuotes) {
    ...
    if (quotes.length > 0) {
      ...
      // Render quote cards
      if (!dryRun) {
        process.stdout.write("   üé® Rendering cards... ");
...
async function main() {
  console.log("üéô  Roadman Content Repurpose Pipeline");
