...
| `rack_rate` | Currency (GBP) | Computed: `base_rate[type] Ã— event_premium[tier]`. Updated when event link changes. |
...
- `rack_rate` recalculated when `event` link changes. No event = base rate (Ã—1.00).
...
|------|-----------|--------|
| 1 | Ã—1.15 | Tour de France, World Championships, Olympics |
| 2 | Ã—1.10 | Giro, Vuelta, Spring Classics block |
| 3 | Ã—1.00 | Monuments, DauphinÃ©, Roadman-owned events |

**Rack Rate Formula:** `rack_rate = BASE_RATES[inventory_type] Ã— EVENT_MULTIPLIERS[event.premium_tier ?? '3']`
...
|---------|-----------|-----------------|------------|-------------|
| Podcast | 3Ã—/week (Mon/Wed/Fri) | 3 (pre/mid/end) | 9 | ~36 |
| Newsletter | 1Ã—/week (Saturday Spin) | 4 (1 dedicated + 1 banner + 2 classified) | 4 | ~16 |
| YouTube | ~2Ã—/week | 1 integration | ~2 | ~8 |
...
| CritÃ©rium du DauphinÃ© 2026 | `classics_block` | 3 |
| Tour de France 2026 | `grand_tour` | 1 |
| Vuelta a EspaÃ±a 2026 | `grand_tour` | 2 |
...
**Episode creation:** `createEpisode()` auto-creates all slot positions (pre/mid/end for podcast, dedicated/banner/classifiedÃ—2 for newsletter).
