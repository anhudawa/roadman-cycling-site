...
│   ├── newsletter/
│   └── strength-training/
├── (content)/             → Content pages with content layout
│   ├── blog/
│   │   ├── page.tsx       → Blog index
│   │   └── [slug]/
│   ├── podcast/
...
│       ├── race-weight/
│       └── fuelling/
├── (community)/           → Community pages with community layout
│   └── community/
...
│   ├── Button.tsx         → Primary (coral), Secondary (purple), Ghost
│   ├── Input.tsx          → Form inputs with floating labels
│   ├── Badge.tsx          → Content pillar tags
│   ├── Card.tsx           → Episode cards, blog cards, tool cards
│   ├── Avatar.tsx         → Guest/member avatars
│   ├── Counter.tsx        → Animated number counters (100M+, 1,852 members)
│   └── Skeleton.tsx       → Loading states
├── layout/                → Structural components
│   ├── Header.tsx         → Sticky header with scroll-aware behaviour
│   ├── Footer.tsx         → Full footer with nav, social, newsletter
│   ├── Section.tsx        → Full-width section wrapper with scroll animations
...
│   │   └── CommunityStats.tsx
│   └── conversion/
│       ├── EmailCapture.tsx    → Inline + modal email capture
│       ├── CTABanner.tsx       → Full-width conversion banners
